...
export default function LeaderboardPage() {
  ...
  // Holds the period the entries were fetched for, so switching periods shows
  // the spinner again without a setState during render or inside an effect body.
  ...
  useEffect(() => {
    ...
    fetch(`/api/leaderboard?period=${period}`)
      ...
      .then((entries: LeaderboardEntry[]) => {
        if (!cancelled) setLoaded({ period, entries });
      })
      .catch((err) => {
        console.error(err);
        if (!cancelled) setLoaded({ period, entries: [] });
