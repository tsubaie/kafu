...
export async function GET(request: NextRequest) {
  try {
    ...
    // Build parameterized query — offset is always the last param for data query
    const searchParam = search ? `%${search}%` : null;
    let paramIndex = 1;

    const searchFilter = search
      ? `AND (u.name ILIKE $${paramIndex++} OR u.email ILIKE $${paramIndex - 1} OR u.department ILIKE $${paramIndex - 1})`
      : "";
    ...
    const dataParams: (string | number)[] = [...countParams, offset];
    ...
    const [dataResult, countResult] = await Promise.all([
      query(
        `SELECT u.id, u.name, u.email, u.department, u.is_active, u.is_admin, u.created_at,
         ...
         ${baseWhere}
         ORDER BY u.name
         LIMIT ${PAGE_SIZE} OFFSET ${offsetParam}`,
        dataParams
      ),
      query(
