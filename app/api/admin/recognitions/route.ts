...
export async function GET(request: NextRequest) {
  try {
    ...
    // Build parameterized query — offset is always the last param
    const searchParam = search ? `%${search}%` : null;
    const dataParams: (string | number)[] = [];
    let paramIndex = 1;

    const searchFilter = search
      ? `AND (s.name ILIKE $${paramIndex++} OR rv.name ILIKE $${paramIndex - 1})`
      : "";
    if (searchParam) dataParams.push(searchParam);
